(() => {
    ...
    (() => {
        ...
        function initSliders() {
            ...
            if (document.querySelector(".three__slider")) new swiper_core_Swiper(".three__slider", {
                modules: [ Navigation ],
                observer: true,
                observeParents: true,
                slidesPerView: 3,
            ...
            if (document.querySelector(".popup-basket__swiper")) new swiper_core_Swiper(".popup-basket__swiper", {
                modules: [ Navigation ],
                observer: true,
                observeParents: true,
                slidesPerView: 1,
        ...
        const btnLogin = document.querySelector(".header__button_login");
        const btnLogout = document.querySelector(".header__button_loguot");
        let isLogin = false;
        const firebaseConfig = {
            apiKey: "AIzaSyDG7JsaSb965kAwzznJtHMvRDL64MYY2vA",
            authDomain: "auto-spares-f5355.firebaseapp.com",
            projectId: "auto-spares-f5355",
            storageBucket: "auto-spares-f5355.appspot.com",
            messagingSenderId: "626402755639",
            databaseURL: "https://auto-spares-f5355-default-rtdb.firebaseio.com",
            appId: "1:626402755639:web:21cd0e8a53edb28c062bb2",
            measurementId: "G-53PS5F21SN"
        };
        ...
        function updateBusketCount(count) {
            let basketBnt = document.querySelector(".header__icon-btn_busket");
            basketBnt.setAttribute("count", count);
        }
        const userID = localStorage.getItem("user") ? JSON.parse(localStorage.getItem("user"))["Ім'я користувача"] : null;
        function logined(status) {
            const buyBtns = document.querySelectorAll(".card__buy-btn");
            const btnBusket = document.querySelector(".header__icon-btn_busket");
            if (status) {
                btnLogout.classList.remove("hide");
                btnLogin.classList.add("hide");
                btnBusket.classList.add("show");
                buyBtns.forEach((btn => {
                ...
                btnBusket.classList.remove("show");
                buyBtns.forEach((btn => {
        ...
        subscribeForms.forEach((form => {
            form.addEventListener("submit", (e => {
                e.preventDefault();
                subscribe(e.submitter);
                form.reset();
            }));
        }));
        const ansubscribeButton = document.querySelectorAll(".ansubscribe-btn");
        ansubscribeButton.forEach((btn => {
            btn.addEventListener("click", (e => {
                writeUserData(`subscribers`, `${userID}`, null);
            }));
        }));
        function subscribe(target) {
            const email = target.parentElement[0];
            writeUserData(`subscribers/${userID}`, `Електронна адреса`, email.value);
            successfull();
        }
        const buyBtns = document.querySelectorAll(".card__buy-btn");
        if (buyBtns) buyBtns.forEach((btn => {
            btn.addEventListener("click", (e => {
                const card = btn.offsetParent.children;
                const id = `b-${btn.offsetParent.id}`;
                const img = card[0].children[0].children[0].src;
                const title = card[1].children[0].innerText;
                const description = card[1].children[1].innerText;
                const price = +card[2].children[1].innerText.split(" ")[0];
                addToBasket(id, img, title, description, price);
                btn.setAttribute("disabled", "disabled");
            }));
        }));
        const telBtn = document.querySelectorAll(".popup-contact__btn");
        if (telBtn) telBtn.forEach((btn => {
            btn.addEventListener("click", (e => {
                const telInput = btn.previousElementSibling;
                writeUserData(`users/${userID}/Контактні дані/`, "Номер телефону", telInput.value);
                successfull();
                setTimeout((() => {
        ...
        function productsGruoped() {
            let cardsAll = document.querySelectorAll(".catalog__cards .catalog__card");
            let cards = document.querySelectorAll(".catalog__cards .catalog__card:not(.hide)");
            const cardsContainer = document.querySelector(".catalog__cards");
            const groups = document.querySelectorAll(".catalog__cards-group");
            if (groups && cardsContainer) {
            ...
            if (cardsContainer) {
                let cardsGroup = [ [] ];
                for (let i = 0, j = 1, g = 0; i < cards.length; i++, j++) {
                    cardsGroup[g].push(cards[i]);
                    if (j >= 4) {
                        j = 0;
                        cardsGroup.push([]);
                        g++;
                ...
                cardsGroup.forEach((i => {
                    ...
                    cardsContainer.append(group);
                }));
                const groups = document.querySelectorAll(".catalog__cards-group");
                document.querySelector(".catalog__cards-group_active") || groups[0].classList.add("catalog__cards-group_active");
                ...
                paginationItems.forEach((item => {
                    item.addEventListener("click", (e => {
                        paginationItems.forEach((i => i.classList.remove("catalog__pagination-item_active")));
                        e.target.classList.add("catalog__pagination-item_active");
                        const itemIndex = item.innerText - 1;
                        groups.forEach((group => group.classList.remove("catalog__cards-group_active")));
                        groups[itemIndex].classList.add("catalog__cards-group_active");
                    }));
                }));
                paginationItems[0].classList.add("catalog__pagination-item_active");
            }
        }
        if (productsGruoped) productsGruoped();
        const sideBtn = document.querySelector(".catalog__side-btn");
        const sideMenu = document.querySelector(".catalog__side-menu");
        if (sideBtn) sideBtn.addEventListener("click", (e => {
            sideMenu.classList.toggle("open");
        }));
        const script_form = document.querySelector(".support__form");
        if (script_form) {
            ...
            function sendMessage() {
                ...
                const message = document.getElementById("support-textarea");
                document.getElementById("support-textarea");
        ...
        const inStock = document.querySelectorAll(".catalog__card:not(.not-avlb)");
